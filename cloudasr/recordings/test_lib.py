...
class TestRecordingsSaver(unittest.TestCase):
    ...
    def test_saver_saves_recordings_received_via_socket(self):
        self.run_saver([
            createSaverMessage(1, 0, 1, 'en-GB', b'body 1', 44100, [(1.0, 'Hello World 1!')], "Lattice").SerializeToString(),
            createSaverMessage(2, 0, 2, 'en-GB', b'body 2', 44100, [(1.0, 'Hello World 2!')], "Lattice").SerializeToString(),
        ])

        self.assertThatSaverSavedRecordings([
            (1, 0, 1, 'en-GB', b'body 1', 44100, [{'confidence': 1.0, 'transcript': 'Hello World 1!'}], "Lattice"),
            (2, 0, 2, 'en-GB', b'body 2', 44100, [{'confidence': 1.0, 'transcript': 'Hello World 2!'}], "Lattice")
        ])
...
class RecordingsModelSpy:
    ...
    def save_recording(self, id, part, chunk_id, model, body, frame_rate, alternatives, lattice):
        self.saved_recordings.append((id, part, chunk_id, model, body, frame_rate, alternatives, lattice))
